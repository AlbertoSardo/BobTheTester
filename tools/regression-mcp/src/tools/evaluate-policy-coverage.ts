...
import { asObjectRecord, asStringArrayStrict, uniqueSorted } from "../utils/helpers.js";
import { matchesPattern, normalizeForMatch } from "../utils/pattern.js";
import { parseCoverageTitle } from "../utils/scaffold.js";
...
async function loadCoverageReport(reportPath: string): Promise<ParsedCoverage | null> {
...
function deriveRiskLevel(overallScore: number): RiskLevel {
  if (overallScore >= 80) {
    return "low";
  }
  if (overallScore >= 60) {
...
export async function evaluatePolicyCoverage(
  ...
  if (Array.isArray(input.changedFiles) && input.changedFiles.length > 0) {
    ...
    changedFiles = uniqueSorted([...changedFilesOutput.changedFiles, ...changedFilesOutput.untrackedFiles]);
  }
  ...
  for (const file of changedFiles) {
    ...
    if (matchedFlows.length > 0) {
      fileToFlows[file] = uniqueSorted(matchedFlows);
    }
  }

  const impactedFlowIds = uniqueSorted(
  ...
  const coverageData = await loadCoverageReport(coverageReportPath);
  if (!coverageData) {
    warnings.push(
  ...
  for (const flowId of impactedFlowIds) {
    ...
    const regressionCov =
      minimumCoverage.length > 0
        ? Math.round((implementedScenarioCount / minimumCoverage.length) * 100)
        : 100;
  ...
  // Compute overall score (average of all flow scores)
  const overallScore =
    flowScores.length > 0
  ...
  return {
    ...
    riskLevel,
    recommendedActions: uniqueSorted(recommendedActions),
